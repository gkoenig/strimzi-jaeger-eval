...
To make it a bit more close to "real-world" scenarios, we are deploying 2 Kafka clusters into 2 different namespaces to show how you can have _testing_ as well as _production_ Kafka cluster within the same K8s cluster, isolated in different namespaces. Let's define that our _testing_ Kafka cluster will be deployed to namespace _testing_ and the **_production_** one into namespace **_kafka-cluster_**. For that we need to create 2 Flux _sources_ (observing the 2 different directories on _main_ branch) and an appropriate directory structure for the kustomization part (see details below).
...
2. create and push the _testing_ branch

    ```bash
    git checkout -b testing
    ```

3. clone **your** Flux Github repo: ```git clone https://github.com/$GITHUB_USER/flux-kafka-demo.git```
4. create a manifest, pointing to **your** strimzi-jaeger-eval repository (since this is the one we want to be observed ;)
  
    > ensure you exported env variable GITHUB_USER with your GitHub username !
...
5. commit and push the -source.yaml
...
6. now that we have only the info about the source git repo configured, let's actually deploy an "application". Flux offers different template mechanisms for that (helm, kustomize, plain yaml manifests,..). We'll use _kustomize_. Here we distinguish between the _testing_ cluster and the _production_ cluster by referencing different paths within our Git Repo. In those paths we then are able to store different kustomizations of the desired Kafka clusters for testing and production.
    ...
    ```bash
    flux create kustomization strimzi-jaeger-eval-testing \
    --source=strimzi-jaeger-eval \
...
7. commit and push the -source.yaml
...
8. now check your cluster for ongoing progress: zookeeper and kafka deployments/service creation
